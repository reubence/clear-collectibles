...
export default function Login() {
  ...
  return (
    <>
      ...
      {loading ? (
        <div
          className={`grid grid-cols-4 lg:grid-cols-11 xl:grid-cols-9 2xl:grid-cols-11 p-6 place-content-between place-items-center absolute inset-0 w-full gap-5 z-10 h-screen min-h-[700px] overflow-y-auto`}
        >
        ...
        <motion.div
          ...
          className={`flex flex-col w-full justify-start items-end bg-[#C7E5F2] py-6 px-6 relative  h-screen min-h-[700px] overflow-x-hidden ${
            shouldApplyScroll ? "overflow-y-auto" : "overflow-y-hidden"
          }`}
        >
          <AnimatePresence>
            <motion.div
          ...
          <div
            className={`flex flex-col  bg-[#15B7FF]/[0.15] justify-between items-center  px-6 pt-8 pb-8 lg:pb-0 z-20 h-[86%]  w-full lg:w-[45%] rounded-2xl mt-6 lg:mt-0 lg:mr-6 ${
              shouldBoxApplyScroll
                ? "lg:max-h-[500px] lg:overflow-y-auto"
                : "max-h-[100%]"
            } ${menuOpen && "bg-white"}`}
          >
            <AnimatePresence>
